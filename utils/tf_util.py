...
def knn(adj_matrix, k=5):
  ...
  """
  neg_adj = -adj_matrix
  _, nn_idx = tf.nn.top_k(neg_adj, k=k+1)  ### remove the current point
  return nn_idx[:, :, 1:]
...
def gather_tensor_along_2nd_axis(batch_input, batch_indices):
    ####2nd [batch npoint dim] --->npoint axis

    [bat_size, point_num, d1] = batch_input.get_shape()
    [_, index_num] = batch_indices.get_shape()
    bat_size_range = tf.range(bat_size)
    bat_size_range_flat = tf.reshape(bat_size_range, [-1, 1])
    bat_size_range_flat_repeat = tf.tile(bat_size_range_flat, [1, int(index_num)])
    bat_size_range_flat_repeat = tf.reshape(bat_size_range_flat_repeat, [-1])
    indices_2d_flat = tf.cast(tf.reshape(batch_indices, [-1]), dtype=tf.int32)
    indices_2d_flat_repeat = bat_size_range_flat_repeat*int(point_num) + indices_2d_flat

    batch_input = tf.reshape(batch_input, [-1, int(d1)])
    batch_input_new = tf.gather(batch_input, indices_2d_flat_repeat)

    batch_input_new = tf.reshape(batch_input_new, [bat_size, int(index_num), int(d1)])
    return batch_input_new


def get_relation_features(point_features, nn_idx, k):

  og_batch_size = point_features.get_shape().as_list()[0]
  point_features = tf.squeeze(point_features)
  if og_batch_size == 1:
      point_features = tf.expand_dims(point_features, 0)

  point_cloud_central = point_features

  point_cloud_shape = point_features.get_shape()
  batch_size = point_cloud_shape[0].value
  num_points = point_cloud_shape[1].value
  num_dims = point_cloud_shape[2].value
  ...
  point_cloud_flat = tf.reshape(point_features, [-1, num_dims])
  point_cloud_neighbors = tf.gather(point_cloud_flat, nn_idx+idx_)
  point_cloud_central = tf.expand_dims(point_cloud_central, axis=-2)
  ...
  for i in range(num_vertex_pairs):
      ...
      temp_concat = tf.concat([point_cloud_neighbors[:, :, vertex_pairs_list[i][0], :],
                               point_cloud_neighbors[:, :, vertex_pairs_list[i][1], :]], axis=-1)

      temp_concat = tf.expand_dims(temp_concat, -2)
      if i == 0:
          relation_features = temp_concat
      else:
          relation_features = tf.concat([relation_features, temp_concat], axis=-2)

  point_features = tf.expand_dims(point_features, axis=-2)
  point_features = tf.tile(point_features, [1, 1, num_vertex_pairs, 1])
  relation_features = tf.concat([point_features, relation_features], axis=-1)
